...
export async function GET(){
  ...
  const [{data:existing,error:existingError},{data:progress},{data:attempts},{data:plan}]=await Promise.all([
  ...
  const rows:any[]=candidates.map((x:any,i:number)=>({user_id:user.id,subject_id:x.topics?.subject_id??null,topic_id:x.topic_id,kind:'weak_topic',title:`${x.topics?.name??'Mavzu'} bo‘yicha practice`,reason:`Aniqlik ${Math.round(Number(x.accuracy)||0)}%. Shu mavzuni yana mashq qilish tavsiya etiladi.`,priority:100-i*10,completed:false}));
  ...
  if(!rows.length&&!(attempts??[]).length)rows.push({user_id:user.id,subject_id:null,topic_id:null,kind:'first_step',title:'Birinchi practice testni boshlang',reason:'Natijalar paydo bo‘lgach, tizim sizga aniqroq tavsiyalar beradi.',priority:50,completed:false});
  if(!rows.length)return NextResponse.json({recommendations:[]});
  const {data:created,error}=await supabase.from('recommendations').insert(rows).select('id,subject_id,topic_id,kind,title,reason,priority,completed,created_at');
