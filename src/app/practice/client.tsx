...
export default function PracticeClient() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      setDurationSeconds(draft.durationSeconds || draft.questions.length * 90);
      const elapsed = Math.floor((Date.now() - (draft.startedAt || Date.now())) / 1000);
      setRemaining(Math.max(0, (draft.durationSeconds || draft.questions.length * 90) - elapsed));
      setStarted(true);
    } catch {}
  }, [draftKey, loading, started, done]);

  useEffect(() => {
    if (started && remaining === 0 && !done) {
      void finishAttempt();
    }
  ...
  const start = async () => {
    ...
    try {
      ...
      const { data, error: rpcError } = await supabase.rpc('practice_questions', {
        p_subject_id: subjectId || null,
        p_topic_id: topicId || null,
        p_difficulty: difficulty || null,
        p_limit: Math.min(Math.max(count, MIN_QUESTIONS), MAX_QUESTIONS),
      });
  ...
  const finishAttempt = async () => {
    ...
    try {
      ...
      await supabase.from('test_attempts').update({
        ...
        score: questions.length ? Math.round((correct / questions.length) * 10000) / 100 : 0,
    ...
    <main className="dashboard-layout">
      ...
      <div className="dashboard-content">
        <header className="dashboard-topbar">
          <div className="dashboard-mobile-brand"><span className="brand-mark">M</span> MilliyTest</div>
          <nav><Link href="/dashboard">Home</Link><Link href="/analytics">Analytics</Link><Link href="/profile">Profil</Link></nav>
        </header>
        <section className="practice-wrap">
          <div className="kicker">PRACTICE · IMTIHON REJIMI</div>
          {!started && !done && (
            <>
              <div className="page-intro"><div><h1>Diqqatni jamlang.</h1><p className="lead">Fan va mavzuni tanlang. Savollar natijangizni avtomatik saqlaydi.</p></div><div className="practice-note">Har bir savol uchun 90 soniya</div></div>
              <div className="form-card practice-start-card">
                ...
                <button className="primary" onClick={start} disabled={starting}>{starting ? 'Tayyorlanmoqda…' : 'Practice boshlash →'}</button>
              </div>
            </>
          )}
          {started && !done && q && (
            <div className="exam-shell">
              ...
              <div className="question-card exam-question">
                <div className="question-meta"><span>{q.difficulty === 'easy' ? 'Oson' : q.difficulty === 'hard' ? 'Qiyin' : 'O‘rta'}</span><span>{Object.keys(answers).length} ta javob saqlangan</span></div>
                <h1>{q.text}</h1>
